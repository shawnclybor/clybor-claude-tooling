...
- **`feature_slug`** — kebab-case identifier (e.g., `oauth-token-refresh`)
- **`problem_statement`** — one paragraph: what's being built and why
- (optional) **`source_material`** — links, screenshots, prior art

If `problem_statement` is missing, ask for it before drafting. Do not infer.

## Output

`docs/PRDs/<feature_slug>.md` with these sections (in order):
...
[1-3 paragraphs. What's the user / system pain? What's the current state? Why now?]

## 2. Success criteria

Each criterion is binary — verifiable by running a command, reading a file, or observing a measurable signal.

- [ ] [Criterion 1, e.g., "Login flow completes in under 200ms P95 across 100 test runs"]
- [ ] [Criterion 2]
- [ ] [Criterion 3]
...
- [What's explicitly NOT included — anti-scope]
...
|---|---|---|
| [Specific failure mode] | common / occasional / rare | [How it gets prevented or handled] |
...
Good criteria (binary):
- "P95 latency on `/login` under 200ms across 100 test requests"
- "`cyclomatic complexity` reported by `radon cc` is ≤ 8 for every function in `auth/`"
- "On invalid email, the API returns HTTP 400 with `{error: 'invalid_email'}` (verified by integration test)"

If a criterion cannot be made binary, it does not belong in the PRD — pull it into `## 7. Open questions` and resolve before drafting.
...
4. Is there a clear out-of-scope section? (Anti-scope prevents scope creep.)
5. Have I listed dependencies even if "none"? (Explicit > implicit.)
...
- Burying solution in the problem — the PRD describes WHAT, not HOW
- Vague risks — "things might break" is not a risk; "the third-party API rate-limits at 100 req/min" is
- Skipping out-of-scope — if you don't write the anti-scope, scope creeps
