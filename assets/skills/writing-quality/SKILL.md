...
machine-generated. This matters because deliverables go to paying clients such as
{{CLIENT_EXAMPLES}} — AI-scented prose undermines the expert authority those clients
are paying for.

## Modes

**`rewrite`** (default) — Flag AI-isms and rewrite the text to fix them. Use during
deliverable curation and email drafting.
...
strength. Consulting deliverables can't afford any AI smell.
...
**`blog`** — Standard long-form prose (proposals, reports not tied to a specific client
deliverable). All rules apply at full strength.

### Auto-detection cues

| Signal | Inferred profile |
|--------|-----------------|
| Content being curated for a client deliverable | `deliverable` |
...
This skill is invoked automatically in these workflows:

1. **Deliverable curation** — after curating content but before any formatting/export step
   in your deliverable workflows. Run in `rewrite` mode with `deliverable` profile.

2. **Client email drafting** — when drafting emails longer than 3 sentences to external
   contacts. Run in `rewrite` mode with `email` profile. Quick internal replies are exempt.

3. **On-demand audit** — when the user says "check this for AI patterns" or similar. Run
   in whichever mode the user requests.

**Correction logging:** when the user corrects a draft this skill produced, record the
correction pattern in {{CORRECTION_LOG}} (a script, log file, or tracker of your choice).
A repeated pattern means the fix belongs at the source — a voice profile or skill
template — not another one-off draft patch.

## Adaptation points

- `{{CLIENT_EXAMPLES}}` — the client names or audience this skill protects (used in the
  framing paragraph; swap in your own)
- `{{CORRECTION_LOG}}` — where repeated correction patterns get recorded
